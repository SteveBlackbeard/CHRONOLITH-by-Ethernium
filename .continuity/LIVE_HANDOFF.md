...
Build Phase 1 of the Continuity governance kernel while keeping AgentOps as a fully separate extractable tool.
...
- AgentOps is isolated in `AGENTOPS_TOOL/` and is not a Continuity dependency.
- Continuity Conekta extraction staging exists in `CONTINUITY_CONEKTA/`.
- The former `nexus-dashboard/` source was extracted to `D:\Experimentos\continuity-conekta`.
...
3. Fix inherited Conekta lint debt in `D:\Experimentos\continuity-conekta`.
4. Consider adding change-contract enforcement for frozen-path baseline refreshes.
