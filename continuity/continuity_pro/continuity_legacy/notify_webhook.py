#!/usr/bin/env python3
"""
notify_webhook.py — CONTINUITY LEGACY v5.1
===========================================
Sends automated notifications to Discord and/or Telegram when the continuity
cycle detects critical events (ATTENTION_REQUIRED, security violations, etc.).

Configure webhook URLs in continuity_legacy.json under the "notifications" key.
...
def _load_webhook_config(repo_root: Path) -> dict:
    """Loads webhook configuration from continuity_legacy.json.
    ...
    """
    config_path = repo_root / "continuity_legacy.json"
...
def send_discord(webhook_url: str, message: str, status: str = "ALERT") -> bool:
    ...
    payload = {
        "username": "CONTINUITY LEGACY",
        "embeds": [{
            "title": f"{emoji} CONTINUITY LEGACY — {status}",
            "description": message,
...
def send_telegram(bot_token: str, chat_id: str, message: str, status: str = "ALERT") -> bool:
    ...
    """
    emoji = {"OK": "✅", "ALERT": "⚠️", "CRITICAL": "🚨"}.get(status, "🔔")
    text = f"{emoji} *CONTINUITY LEGACY — {status}*\n\n{message}\n\n_Ethernium Hub_"
...
def notify(repo_root: Path, message: str, status: str = "ALERT") -> dict:
    ...
    Reads webhook configuration automatically from continuity_legacy.json.
    ...
    if discord_url:
        ...
        print("  [~] Discord not configured (set notifications.discord_webhook_url in continuity_legacy.json).")
...
if __name__ == "__main__":
    ...
    notify(root, "This is a test notification from CONTINUITY LEGACY v5.1.", "ALERT")
