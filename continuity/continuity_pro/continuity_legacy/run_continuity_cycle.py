# Standard imports at top, heavy imports inside main
import sys
import argparse
import json
import os
...
def run_continuity_cycle(repo_root: str | Path, external_root_override: str | Path | None = None) -> dict:
    from core.automation_common import bootstrap_output_path, continuity_report_path, load_config, read_text, resolve_repo_root, state_path, utc_now_iso
...
def main() -> None:
    ...
    if args.hook:
        ...
        if not (repo_root / ".git").exists():
            echo("[!] ERROR: Not a git repository. Cannot install hooks.", Color.RED)
            return
        
        hook_content = f"#!/bin/sh\n# Continuity Pro Auto-Hook\npython {Path(__file__).resolve()} --strict || exit 1\n"
        hook_path.parent.mkdir(parents=True, exist_ok=True)
        hook_path.write_text(hook_content, encoding="utf-8")
        if os.name != "nt": os.chmod(hook_path, 0o755)
        echo(f"[✔] Push Hook (Strict Mode) installed at {hook_path}", Color.GREEN)
        return

    # 0.5 Self-Healing (Interactively ensure core files)
    def ensure_pro_file(path: Path, template: str, desc: str):
        if not path.exists():
            echo(f"[?] Missing {desc} ({path.name})", Color.CYAN)
            try:
                if sys.stdin.isatty():
                    choice = input("    -> Create basic template? [Y/n]: ").strip().lower()
                    if choice in ["", "y", "yes"]:
                        path.parent.mkdir(parents=True, exist_ok=True)
    
                        # Ready to write template
    
                        # Initialize Store with Multi-Project support
                        path.write_text(template, encoding="utf-8")
                        echo(f"    [✔] Created {path.name}", Color.GREEN)
            except (EOFError, KeyboardInterrupt): pass

    ensure_pro_file(repo_root / "PROJECT_CONTEXT.md", "# Project Context\n\n- Define your strategic intent here.", "Project Context")
    ensure_pro_file(repo_root / ".continuity" / "DECISIONS_LOG.md", "# Decision Log\n\n| Date | Decision | Rationale | Actor |\n| :--- | :--- | :--- | :--- |\n", "Decision Log")
