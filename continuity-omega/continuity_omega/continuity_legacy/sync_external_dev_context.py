...
def _build_readme(snapshot: dict, ext_name: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "Purpose:",
            "- external developer continuity layer",
...
def _build_canonical_paths(repo_root: Path, ext_root: Path) -> str:
    lines = [
        ...
        f"- {repo_root / '.continuity'}",
        "",
        "OUTPUTS",
        f"- {repo_root / 'outputs' / 'continuity' / 'context_bootstrap_summary.json'}",
        f"- {repo_root / 'outputs' / 'continuity' / 'continuity_cycle_report.json'}",
        "",
        "EXTERNAL DOCS",
        f"- {ext_root}",
    ]
...
def _build_automation_cycle(repo_root: Path) -> str:
    command = f"python tools/continuity_legacy/run_continuity_cycle.py --repo-root {repo_root} --strict"
    return "\n".join(
        [
            "AUTOMATION CYCLE",
            "",
            ...
            "",
            "WHAT IT DOES",
            "1. rebuilds the internal continuity bootstrap snapshot",
            ...
            "6. writes a continuity report",
        ]
    )


def _build_external_handoff(repo_root: Path, snapshot: dict) -> str:
    handoff_path = repo_root / ".continuity" / "LIVE_HANDOFF.md"
...
def sync_external_dev_context(repo_root: str | Path, external_root_override: str | Path | None = None) -> dict:
    ...
    _write_text(Path(ext_root) / "03_HISTORY" / "DECISIONS_LOG.txt", _build_history_copy(repo_root, ".continuity/DECISIONS_LOG.md", "DECISIONS LOG"))
    _write_text(Path(ext_root) / "03_HISTORY" / "TIMELINE.txt", _build_history_copy(repo_root, ".continuity/TIMELINE.md", "TIMELINE"))

    return {"status": "ok", "external_root": str(ext_root)}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sync optional external developer continuity docs.")
