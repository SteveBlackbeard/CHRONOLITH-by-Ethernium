...
# CHRONOLITH OMEGA (v2.1.0) - Celestial Cognitive Oracle
# -------------------------------------------------------------
# [!] Industrial Grade Refactor: Typer CLI, Rich UI, RAG, Cognitive Maps.

app = typer.Typer(
    help=f"{OMEGA_ICON} Chronolith Omega: The pinnacle AI oracle. Advanced RAG and cognitive mapping.",
...
def setup_logger(repo_root: Path):
    ...
    log_file = log_dir / f"omega_chronolith_{datetime.now().strftime('%Y%m%d')}.jsonl"
    ...
    logger = logging.getLogger("chronolith_omega")
...
def init(
    ...
    files = {
        ".chronolith/STATE.json": '{"phase": "omega", "last_update": "' + datetime.utcnow().isoformat() + '"}',
...
def query(
    ...
    results = omega_engine.query_chronolith(store, q)
...
def map(
    ...
    log = root / ".chronolith" / "DECISIONS_LOG.md"
    if not log.exists():
