...
def _get_client(repo_root):
    import chromadb
    persist_path = Path(repo_root) / ".chronolith" / "omega_index"
...
def initialize_omega_store(repo_root, external_roots=None):
    ...
    if external_roots:
        for ext_path in external_roots:
            ...
            if (ext_path / ".chronolith" / "omega_index").exists():
...
def generate_project_dna(repo_root, collection):
    ...
    dna_path = Path(repo_root) / ".chronolith" / "PROJECT_DNA.md"
    dna_content = [
...
def query_chronolith(collection, query_text):
