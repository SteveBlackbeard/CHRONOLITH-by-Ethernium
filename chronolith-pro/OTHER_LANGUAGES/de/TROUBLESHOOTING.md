# CHRONOLITH: Leitfaden zur Fehlerbehebung 🛠️
...
*   **Lösung**:
    ...
    1.  Prüfen Sie den Bericht in `outputs/chronolith/chronolith_cycle_report.json`, um zu sehen, welcher "required_string" fehlt.

    2.  Fügen Sie die Paritätsmarkierung wieder in das Dokument ein.

    3.  Führen Sie `python tools/chronolith/run_chronolith_cycle.py` erneut aus.
...
*   **Lösung**:
    ...
    1.  Führen Sie `python tools/chronolith/chronolith_status.py` aus, um das Health Dashboard zu sehen.

    2.  Synchronisieren Sie den Status mit `python tools/chronolith/run_chronolith_cycle.py`.
...
*   **Lösung**:
    ...
    2.  Verwenden Sie `python tools/chronolith/chronolith_suggest.py`, um eine gute Zusammenfassung der bisherigen Ereignisse zu erstellen und der KI zu übergeben.
...
*   **Ursache**: Sie versuchen, die Skripte außerhalb eines gültigen **CHRONOLITH**-Repositorys auszuführen.

*   **Lösung**:

    1.  Stellen Sie sicher, dass Sie sich im Projektstammverzeichnis befinden.

    2.  Prüfen Sie, ob die Datei `chronolith.json` oder der Ordner `.chronolith` existiert.
...
## 5. Dashboard (`chronolith_status`) zeigt "Unknown" oder "Skipped" ❓
...
*   **Lösung**:
    ...
    1.  Führen Sie den Zyklus aus: `python tools/chronolith/run_chronolith_cycle.py`.
