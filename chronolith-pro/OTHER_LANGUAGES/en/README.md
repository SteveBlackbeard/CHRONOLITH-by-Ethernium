...
`CHRONOLITH` is a standalone starter toolkit for building projects with persistent chronolith, canonical memory, and repeatable handoff between humans and AI operators.



This toolkit is chronolith-first: it provides a reusable discipline for context persistence, document parity, and governed handoff without depending on any external framework.
...
- a chronolith bootstrap snapshot
...
- a strict chronolith closeout command
...
pip install chronolith-legacy

chronolith-legacy init "My Project"
...
python tools/chronolith/bootstrap_project.py --repo-root . --project-name "My Project" --project-slug my_project

```



3. If you want an external chronolith layer:



```powershell

python tools/chronolith/bootstrap_project.py --repo-root . --project-name "My Project" --project-slug my_project --enable-external-docs

```



## Automation & Safety (Chronolith Guard)
