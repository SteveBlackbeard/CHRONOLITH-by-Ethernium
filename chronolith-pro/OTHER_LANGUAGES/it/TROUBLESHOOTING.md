# CHRONOLITH: Guida alla Risoluzione dei Problemi 🛠️
...
*   **Soluzione**:
    ...
    1.  Controlla il report in `outputs/chronolith/chronolith_cycle_report.json` per vedere quale "required_string" manca.

    2.  Aggiungi di nuovo il marcatore di parità nel documento.

    3.  Esegui nuovamente `python tools/chronolith/run_chronolith_cycle.py`.
...
*   **Soluzione**:
    ...
    1.  Esegui `python tools/chronolith/chronolith_status.py` per vedere la Dashboard della salute.

    2.  Sincronizza lo stato usando `python tools/chronolith/run_chronolith_cycle.py`.
...
*   **Soluzione**:
    ...
    2.  Usa `python tools/chronolith/chronolith_suggest.py` per generare un buon riassunto di quanto accaduto e fornirlo all'IA.
...
*   **Causa**: Stai cercando di eseguire gli script al di fuori di un repository valido di **CHRONOLITH**.

*   **Soluzione**:

    1.  Assicurati di essere nella radice del progetto.

    2.  Verifica che esista il file `chronolith.json` o la cartella `.chronolith`.
...
## 5. La Dashboard (`chronolith_status`) mostra "Unknown" o "Skipped" ❓
...
*   **Soluzione**:
    ...
    1.  Esegui il ciclo: `python tools/chronolith/run_chronolith_cycle.py`.
