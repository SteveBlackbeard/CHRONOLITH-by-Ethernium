# CHRONOLITH: Guía de Solución de Problemas (Troubleshooting) 🛠️
...
*   **Solución**:
    ...
    1.  Revisa el reporte en `outputs/chronolith/chronolith_cycle_report.json` para ver qué "required_string" falta.

    2.  Añade el marcador de paridad de nuevo en el documento.

    3.  Vuelve a ejecutar `python tools/chronolith/run_chronolith_cycle.py`.
...
*   **Solución**:
    ...
    1.  Ejecuta `python tools/chronolith/chronolith_status.py` para ver el Dashboard de salud.

    2.  Sincroniza el estado usando `python tools/chronolith/run_chronolith_cycle.py`.
...
*   **Solución**:
    ...
    2.  Usa `python tools/chronolith/chronolith_suggest.py` para generar un buen resumen de lo que ha pasado y dárselo a la IA.
...
*   **Causa**: Estás intentando ejecutar los scripts fuera de un repositorio válido de **CHRONOLITH**.

*   **Solución**:

    1.  Asegúrate de estar en la raíz del proyecto.

    2.  Comprueba que existe el archivo `chronolith.json` o la carpeta `.chronolith`.
...
## 5. El Dashboard (`chronolith_status`) muestra "Unknown" o "Skipped" ❓
...
*   **Solución**:
    ...
    1.  Ejecuta el ciclo: `python tools/chronolith/run_chronolith_cycle.py`.
