...
1. **インストール**: `tools/chronolith/` フォルダをご自身のプロジェクトにコピーしてください。

2. **インテリジェントな初期化 (グラスボックス)**:

```powershell

python tools/chronolith/bootstrap_project.py --repo-root . --project-name "My Project" --project-slug my_project --discover
...
python tools/chronolith/bootstrap_project.py --project-name "AI Core" --project-slug ia_core --with-vector



# メモリグラフ (NetworkX) サポートを追加

python tools/chronolith/bootstrap_project.py --project-name "Brain" --project-slug brain --with-graph
...
python tools/chronolith/run_chronolith_cycle.py --strict
