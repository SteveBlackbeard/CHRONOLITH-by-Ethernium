# CHRONOLITH: Guide de Dépannage (Troubleshooting) 🛠️
...
*   **Solution** :
    ...
    1.  Vérifiez le rapport dans `outputs/chronolith/chronolith_cycle_report.json` pour voir quelle "required_string" manque.

    2.  Ajoutez le marqueur de parité à nouveau dans le document.

    3.  Exécutez `python tools/chronolith/run_chronolith_cycle.py` à nouveau.
...
*   **Solution** :
    ...
    1.  Exécutez `python tools/chronolith/chronolith_status.py` pour voir le tableau de bord de santé.

    2.  Synchronisez l'état à l'aide de `python tools/chronolith/run_chronolith_cycle.py`.
...
*   **Solution** :
    ...
    2.  Utilisez `python tools/chronolith/chronolith_suggest.py` pour générer un bon résumé de ce qui s'est passé et donnez-le à l'IA.
...
*   **Cause** : Vous essayez d'exécuter les scripts en dehors d'un dépôt valide de **CHRONOLITH**.

*   **Solution** :

    1.  Assurez-vous d'être à la racine du projet.

    2.  Vérifiez que le fichier `chronolith.json` ou le dossier `.chronolith` existe.
...
## 5. Le tableau de bord (`chronolith_status`) affiche "Unknown" ou "Skipped" ❓
...
*   **Solution** :
    ...
    1.  Exécutez le cycle : `python tools/chronolith/run_chronolith_cycle.py`.
