...
1.  Ejecutar el `decision_engine.py`: 
    ...
    python tools/chronolith/decision_engine.py

    ```

2.  Leer el archivo `outputs/chronolith/strategic_decision.json`.
...
1. `outputs/chronolith/memory_summary.md` (Read first for a high-level overview).
...
5. `.chronolith/LIVE_HANDOFF.md` (The exact point where the last agent left off).



## 2. Mandatory Memory Management 🧩

As an agent of Chronolith, you are responsible for the health of the project's memory.
...
- **Summarize Early, Summarize Often**: If a session becomes too long, run `python tools/chronolith/summarize_memory.py` and refer to the summary to reduce context drift.

- **Update the Handoff**: Every significant action must be recorded in the `.chronolith/` surfaces.



## 3. Logical Immunity Workflow 🛡️

This project uses a **Strict Chronolith Cycle**. Your work will be blocked by the Git Hooks if you break the structural integrity of the documentation.



- Run `python tools/chronolith/run_chronolith_cycle.py --strict` regularly.
...
| **Initialize Repo** | `python tools/chronolith/bootstrap_project.py --repo-root . --discover` |

| **Check Health** | `python tools/chronolith/chronolith_status.py` |

| **Run Cycle** | `python tools/chronolith/run_chronolith_cycle.py --strict` |

| **Summarize Memory** | `python tools/chronolith/summarize_memory.py` |
