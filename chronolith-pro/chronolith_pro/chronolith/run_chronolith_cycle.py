#!/usr/bin/env python3
"""
run_chronolith_cycle.py — CHRONOLITH Pro (v3.0.3)
...
# Figlet generated with pyfiglet (slant), not hand-drawn — a hand-edited banner
# is how the previous one silently lost letters. {version} is filled from
# _get_version() at print time so it never goes stale.
ASCII_ART = """
[bold magenta]
   ________  ______  ____  _   ______  __    ____________  __
  / ____/ / / / __ \\/ __ \\/ | / / __ \\/ /   /  _/_  __/ / / /
 / /   / /_/ / /_/ / / / /  |/ / / / / /    / /  / / / /_/ /
/ /___/ __  / _, _/ /_/ / /|  / /_/ / /____/ /  / / / __  /
\\____/_/ /_/_/ |_|\\____/_/ |_/\\____/_____/___/ /_/ /_/ /_/

      PRO [bold white]v{version}[/bold white] | [italic magenta]Cryptographic integrity for project context[/italic magenta]
[/bold magenta]
"""
...
def setup_logger(repo_root: Path):
    ...
    log_file = log_dir / f"pro_chronolith_{datetime.now().strftime('%Y%m%d')}.jsonl"
...
def _get_version() -> str:
    ...
    try:
        return importlib.metadata.version("chronolith-pro")
...
def crystallize_readme(repo_root: Path, merkle_root: str):
    ...
    if marker in content:
        ...
        crystal_text = (
            ...
            f"> [![Merkle Root](https://img.shields.io/badge/DNA--Crystallized-{merkle_root[:8]}-magenta)](https://github.com/SteveBlackbeard/CHRONOLITH-by-Ethernium)\n"
        )
...
def init(
    ...
    files = {
        ...
        ".chronolith/STATE.json": json.dumps({"phase": "pro", "last_update": now_iso}, indent=2),
        ".chronolith/DECISIONS_LOG.md": "# Decision Log\n\n| Date | Decision | Rationale | Actor |\n| :--- | :--- | :--- | :--- |\n",
...
def check(
    ...
    # signed baseline in .chronolith/STATE.json is verified and compared.
    ...
    trusted_keys = (
        sovereign_vault.historically_valid_pubkeys(root / ".chronolith" / "keys", public_bytes)
    ...
    if not dna_drift and not signature_tampered and chain_ok:
        ...
        if private_bytes or not public_bytes:
            ...
            console.print("[yellow][i] Vault locked (CHRONOLITH_PASSPHRASE unset): chain entry postponed until the key is available.[/yellow]")
    ...
    # Save Report
    report_path = root / ".chronolith" / "pro_report.json"
...
def sovereign_init(
    repo_root: Path = typer.Option(".", "--repo-root", help="Project root directory."),
    encrypt: bool = typer.Option(False, "--encrypt", help="Protect the private keys at rest with a passphrase (CHRONOLITH_PASSPHRASE env var; scrypt + ChaCha20-Poly1305)."),
):
    ...
    if encrypt:
        passphrase = os.environ.get("CHRONOLITH_PASSPHRASE", "")
        if not passphrase:
            console.print("[bold red][!] --encrypt requires the CHRONOLITH_PASSPHRASE environment variable.[/bold red]")
            raise typer.Exit(code=1)
        for plain in (priv_path, seal_priv):
            ...
            plain.unlink()
        vault_note = "ENCRYPTED at rest (scrypt + ChaCha20-Poly1305; unlock via CHRONOLITH_PASSPHRASE)"
...
def sovereign_rotate(
    ...
    if not (old_priv and old_pub):
        console.print("[bold red][!] No unlockable sovereign keypair to rotate (missing keys or locked vault without CHRONOLITH_PASSPHRASE).[/bold red]")
        raise typer.Exit(code=1)
...
def chain(
    ...
    if verify:
        _priv, pub = automation_common.load_sovereign_keys(root)
        keys = sovereign_vault.historically_valid_pubkeys(root / ".chronolith" / "keys", pub) if pub else None
...
def verify_proof(
    ...
    if not expected_root:
        state_file = root / ".chronolith" / "STATE.json"
...
def verify(
    ...
    trusted = sovereign_vault.historically_valid_pubkeys(root / ".chronolith" / "keys", pub) if pub else None
    ...
    anchors = sorted((root / ".chronolith" / anchor_mod.ANCHOR_DIRNAME).glob("*.json.ots")) if (root / ".chronolith" / anchor_mod.ANCHOR_DIRNAME).exists() else []
    if anchors:
...
def upgrade_anchors(
    ...
    root = repo_root.resolve()
    directory = root / ".chronolith" / anchor_mod.ANCHOR_DIRNAME
...
def anchor(
    ...
    if stamped:
        console.print(Panel(
            ...
            f"Confirmation takes hours. Verify later: [cyan]chronolith-pro verify-anchor --proof {str(anchor_path) + '.ots'}[/cyan]\n"
        ...
        console.print("[dim]Frictionless stamp:  pip install chronolith-pro[anchor]  (pure-Python, no ots CLI)")
...
def log(
    ...
    root = repo_root.resolve()
    log_path = root / ".chronolith" / "DECISIONS_LOG.md"
...
def demo(
    ...
    console.print("[bold magenta]⚡ ETHERNIUM CHRONOLITH — DRIFT DETECTION DEMO[/bold magenta]")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            console.print("    [yellow]⚠ Git would accept this change silently. Chronolith will not.[/yellow]")
            time.sleep(2)
            ...
            if merkle_before != merkle_after:
                ...
                console.print()
                console.print("[bold green]✅ THE GUARDIAN WORKS.[/bold green] Chronolith detected")
            ...
            console.print("[bold magenta]🔥 Chronolith: AI doesn't forget anymore.[/bold magenta]")
            console.print()
            console.print("Next steps:")
            console.print("  [dim]1.[/dim] [bold]chronolith init[/bold] — Initialize in your real project")
            console.print("  [dim]2.[/dim] [bold]chronolith check[/bold] — Validate your project DNA")
            console.print("  [dim]3.[/dim] [bold]chronolith log \"my intent\"[/bold] — Log design decisions")
...
@app.command()
def version():
    """Show the Chronolith Pro version and Merkle status."""
    ver = _get_version()
    console.print(f"[bold magenta]Chronolith Pro[/bold magenta] v{ver}")
