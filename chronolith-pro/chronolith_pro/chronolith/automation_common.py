...
CONFIG_FILENAME = "chronolith.json"
_CONFIG_FILENAME_LEGACY = "chronolith.json"
...
def resolve_repo_root(repo_root: str | Path | None, current_file: str | Path) -> Path:
    ...
    for parent in [start_path] + list(start_path.parents):
        if (parent / CONFIG_FILENAME).exists() or (parent / _CONFIG_FILENAME_LEGACY).exists() or (parent / ".chronolith").exists():
            return parent
...
def load_sovereign_keys(repo_root: str | Path) -> tuple[bytes | None, bytes | None]:
    """Load raw Ed25519 key material from <root>/.chronolith/keys (repo-rooted,
    ...
    `sovereign-init --encrypt`) is decrypted via the CHRONOLITH_PASSPHRASE
    environment variable so non-interactive hooks keep working. A plaintext
    sovereign.priv is used as-is."""
    import os

    key_dir = Path(repo_root) / ".chronolith" / "keys"
...
def chain_path(repo_root: str | Path) -> Path:
    return Path(repo_root) / ".chronolith" / CHAIN_FILENAME
...
def load_hash_cache(repo_root: str | Path) -> dict:
    return read_json(Path(repo_root) / ".chronolith" / HASH_CACHE_FILENAME, {}) or {}


def save_hash_cache(repo_root: str | Path, cache: dict) -> None:
    write_json(Path(repo_root) / ".chronolith" / HASH_CACHE_FILENAME, cache)
