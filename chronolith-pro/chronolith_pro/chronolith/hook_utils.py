...
def get_abs_script() -> str:
    ...
    script_path = Path(__file__).resolve().parent.parent / "run_chronolith_pro.py"
    if not script_path.exists():
        # Fallback to local dev path
        script_path = Path(__file__).resolve().parent / "run_chronolith_cycle.py"
    return script_path.as_posix()

def get_pre_commit_content() -> str:
    return f"""#!/bin/sh
# CHRONOLITH PRE-COMMIT HOOK (Militarized Fail-Closed)
...
if [ $RESULT -ne 0 ]; then
  echo "[!] CHRONOLITH ERROR: Project state has drift."
...
def get_pre_push_content() -> str:
    return f"""#!/bin/sh
# CHRONOLITH PRE-PUSH HOOK (Militarized Fail-Closed)
