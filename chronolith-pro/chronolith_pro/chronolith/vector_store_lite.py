...
logger = logging.getLogger("CHRONOLITH_LEGACY_VECTOR")

class ChronolithVectorStore:
    """
    A professional wrapper for vector-based memory stores.
    Targets ChromaDB for simplicity and local execution.
    """
    def __init__(self, db_path: str = ".chronolith/vector_db"):
    ...
    def _ensure_chroma(self):
        ...
        if self._client is None:
            try:
                ...
                self._collection = self._client.get_or_create_collection(name="chronolith_memory")
            except ImportError:
