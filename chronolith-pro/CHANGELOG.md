...
- **Config filename mismatch** (`chronolith.json` vs `chronolith.json`) resolved with a tolerant resolver; `resolve_repo_root` no longer risks an `IndexError` on shallow trees.
...
- **`sovereign-init`** (`--encrypt`): Ed25519 signing + X25519 encryption keys; optional scrypt + ChaCha20-Poly1305 vault unlocked via `CHRONOLITH_PASSPHRASE`.
...
- **Added**: `tools/chronolith/heal_parity.py` - Self-healing engine for document markers.

- **Improved**: `run_chronolith_cycle.py` with actionable healing suggestions and detailed notifications.

- **Added**: Automated PyPI release workflow in `.github/workflows/chronolith.yml`.
...
- **Added**: `chronolith_status.py` - Color-coded health dashboard for the project.

- **Added**: `chronolith_suggest.py` - AI-assisted update suggestions based on git diffs.

- **Improved**: Modular context loading with `[include: path/to/file.md]` support.

- **Improved**: `.chronolithignore` support for parity checks.



## [1.0.0] - 2026-03-31: Initial Release () 💎

- First manual framework for chronolith and document parity.

- Core logic for `STATE.json`, `ROADMAP.md`, and `.chronolith/` surfaces.
