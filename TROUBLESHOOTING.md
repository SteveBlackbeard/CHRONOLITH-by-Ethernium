# CONTINUITY LEGACY: Solución de Problemas (Troubleshooting) 🛠️
...
*   **Causa**: Has editado un archivo (ej: el README o el Handoff) y has borrado sin querer una línea que el sistema monitoriza para asegurar la coherencia.
*   **Solución**:
...
## 2. El Git Hook bloquea mi commit/push 🛡️
...
## 3. Mi agente AI parece "perdido" o ignora el contexto 🤖
...
*   **Solución**:
    1.  Asegúrate de entregarle el archivo **`AGENT_START.md`** al principio de la sesión.
    ...
    3.  Pídele a la IA: *"Reconstruye tu estado actual leyendo el STATE.json y dime cuál es tu Next Exact Action"*.

---

## 4. Error de "Security Warning" al iniciar ⚠️

*   **Problema**: Los scripts de Python lanzan un error de seguridad al intentar resolver la ruta de la raíz.
...
*   **Solución**:
    ...
    2.  Verifica que existe el archivo `continuity_legacy.json` o la carpeta `.continuity`.
    3.  Si has copiado el proyecto a mano, asegúrate de correr el `bootstrap_project.py` primero.
...
*   **Causa**: No has completado un ciclo completo de continuidad o la característica de "External Docs" está desactivada.
*   **Solución**:
