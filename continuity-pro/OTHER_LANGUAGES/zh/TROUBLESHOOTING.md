...
*   **解决方案**:
    ...
    1.  查看 `outputs/continuity/continuity_cycle_report.json` 以查看哪个“required_string”缺失。

    2.  将一致性标记重新添加回文档。

    3.  再次运行 `python tools/continuity_legacy/run_continuity_cycle.py`。
...
*   **解决方案**:
    ...
    1.  运行 `python tools/continuity_legacy/continuity_status.py` 以查看健康仪表板。

    2.  使用 `python tools/continuity_legacy/run_continuity_cycle.py` 同步状态。
...
*   **解决方案**:
    ...
    2.  使用 `python tools/continuity_legacy/continuity_suggest.py` 生成已发生事情的良好摘要并提供给 AI。
...
*   **原因**: 你尝试在有效的 **CONTINUITY LEGACY** 存储库之外运行脚本。

*   **解决方案**:

    1.  确保你位于项目根目录。

    2.  检查 `continuity_legacy.json` 文件或 `.continuity` 文件夹是否存在。
...
## 5. 仪表板 (`continuity_status`) 显示“Unknown”或“Skipped” ❓
...
*   **解决方案**:
    ...
    1.  运行周期： `python tools/continuity_legacy/run_continuity_cycle.py`。
