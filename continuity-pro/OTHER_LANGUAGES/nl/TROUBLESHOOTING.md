# CONTINUITY LEGACY: Gids voor Probleemoplossing 🛠️
...
*   **Oplossing**:
    ...
    1.  Bekijk het rapport in `outputs/continuity/continuity_cycle_report.json` om te zien welke "required_string" ontbreekt.

    2.  Voeg de pariteitsmarkering weer toe aan het document.

    3.  Voer `python tools/continuity_legacy/run_continuity_cycle.py` opnieuw uit.
...
*   **Oplossing**:
    ...
    1.  Voer `python tools/continuity_legacy/continuity_status.py` uit om het dashboard voor gezondheidscontrole te bekijken.

    2.  Synchroniseer de status met `python tools/continuity_legacy/run_continuity_cycle.py`.
...
*   **Oplossing**:
    ...
    2.  Gebruik `python tools/continuity_legacy/continuity_suggest.py` om een goede samenvatting te genereren van wat er is gebeurd en geef deze aan de AI.
...
*   **Oorzaak**: Je probeert de scripts uit te voeren buiten een geldige **CONTINUITY LEGACY**-repository.

*   **Oplossing**:

    1.  Zorg ervoor dat je je in de hoofdmap van het project bevindt.

    2.  Controleer of het bestand `continuity_legacy.json` of de map `.continuity` bestaat.
...
## 5. Dashboard (`continuity_status`) Toont "Unknown" of "Skipped" ❓
...
*   **Oplossing**:
    ...
    1.  Voer de cyclus uit: `python tools/continuity_legacy/run_continuity_cycle.py`.
