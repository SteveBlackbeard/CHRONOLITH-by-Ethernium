# CONTINUITY LEGACY: トラブルシューティングガイド 🛠️
...
*   **解決策**:
    ...
    1.  `outputs/continuity/continuity_cycle_report.json` のレポートを確認して、どの「required_string」が欠落しているかを特定します。

    2.  整合性マーカーをドキュメントに戻します。

    3.  `python tools/continuity_legacy/run_continuity_cycle.py` を再度実行します。
...
*   **解決策**:
    ...
    1.  `python tools/continuity_legacy/continuity_status.py` を実行して、ヘルスダッシュボードを確認します。

    2.  `python tools/continuity_legacy/run_continuity_cycle.py` を使用して、状態を同期します。
...
*   **解決策**:
    ...
    2.  `python tools/continuity_legacy/continuity_suggest.py` を使用して、何が起きたかの概要（サマリー）を生成し、AIに提供します。
...
*   **原因**: 有効な **CONTINUITY LEGACY** リポジトリの外でスクリプトを実行しようとしています。

*   **解決策**:

    1.  プロジェクトのルートにいることを確認します。

    2.  `continuity_legacy.json` ファイルまたは `.continuity` フォルダが存在することを確認します。
...
## 5. ダッシュボード (`continuity_status`) が 「Unknown」 または 「Skipped」 ❓
...
*   **解決策**:
    ...
    1.  サイクルを実行します： `python tools/continuity_legacy/run_continuity_cycle.py`。
