# Sovereign Security (Continuity Pro)
...
| History integrity | `chain` | Rewriting the past: every crystallization is hash-linked and signed in `.continuity/dna_chain.jsonl`. |
...
#    Add --encrypt to protect the private keys with CONTINUITY_PASSPHRASE.
continuity-pro sovereign-init --encrypt

# 2. Crystallize + sign the baseline (this is your normal guardian run).
continuity-pro check --strict

# 3. Inspect and verify the transparency chain.
continuity-pro chain

# 4. Prove one file belongs to the DNA, then verify the proof.
continuity-pro prove --file README.md --output readme.proof.json
continuity-pro verify-proof --proof readme.proof.json
...
- **Non-interactive unlock.** An encrypted vault is opened via the
  `CONTINUITY_PASSPHRASE` environment variable, so CI and pre-push hooks keep
...
- **Rotation.** `sovereign-rotate` generates a new key and has the **old key
  sign the hand-off** into `.continuity/keys/rotations.jsonl`. Verification walks
