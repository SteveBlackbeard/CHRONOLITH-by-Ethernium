# CONTINUITY LEGACY: Troubleshooting Guide 🛠️

Something doesn't match? Don't panic. Here are the solutions to common continuity issues.
...
*   **Solution**:
    1.  Check the report in `outputs/continuity/continuity_cycle_report.json` to see which "required_string" is missing.
    2.  Add the parity marker back into the document.
    3.  Run `python tools/continuity_legacy/run_continuity_cycle.py` again.
...
*   **Solution**:
    1.  Run `python tools/continuity_legacy/continuity_status.py` to see the Health Dashboard.
    2.  Sync the state using `python tools/continuity_legacy/run_continuity_cycle.py`.
...
*   **Solution**:
    ...
    2.  Use `python tools/continuity_legacy/continuity_suggest.py` to generate a good summary of what has happened and give it to the AI.
...
*   **Cause**: You are trying to run the scripts outside of a valid **CONTINUITY LEGACY** repository.
*   **Solution**:
    1.  Ensure you are in the project root.
    2.  Check that the `continuity_legacy.json` file or `.continuity` folder exists.
    3.  If you copied the project manually, ensure you run `bootstrap_project.py` first.

---

## 5. Dashboard (`continuity_status`) Shows "Unknown" or "Skipped" ❓

*   **Problem**: Some section of the health system shows no data.
*   **Cause**: You haven't completed a full continuity cycle or the "External Docs" feature is disabled.
*   **Solution**:
    1.  Run the cycle: `python tools/continuity_legacy/run_continuity_cycle.py`.
