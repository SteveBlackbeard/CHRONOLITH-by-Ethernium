...
Write-Host "[*] CHRONOLITH Lite - Installer"
...
Write-Host "[->] Installing zero-overhead chronolith into '$targetDir'..."
...
# The run script
$runScriptSrc = Join-Path -Path $PSScriptRoot -ChildPath "run_chronolith_lite.py"
$runScriptDest = Join-Path -Path $targetDir -ChildPath "run_chronolith_lite.py"

if (Test-Path $runScriptSrc) {
    Copy-Item -Path $runScriptSrc -Destination $runScriptDest -Force
} else {
    Write-Host "[!] Could not find 'run_chronolith_lite.py' to copy. Ensure it exists next to this installer." -ForegroundColor Red
    exit 1
}
...
Write-Host "[✔] CHRONOLITH Lite installed successfully!" -ForegroundColor Green
Write-Host "    To use it, run 'python run_chronolith_lite.py' inside your project root."
