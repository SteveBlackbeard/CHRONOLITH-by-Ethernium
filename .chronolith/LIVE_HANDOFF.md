# Chronolith Live Handoff

## Current Objective

Build Phase 1 of the Chronolith governance kernel while keeping ROBIN HOOD and CONEKTA as external tools.

## Current State

- Governance rulebook exists at `.chronolith/rulebook.json`.
- Feature registry exists at `.chronolith/feature-registry.json`.
...
`python scripts/autophagy_report.py` ignores extractable external tools and currently identifies remaining Chronolith attention targets.

The main remaining non-core attention targets are:

- `_tmp_reglas_oro/reglas de oro/10_CHRONOLITH.md`
